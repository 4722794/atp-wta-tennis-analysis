...
# All mens data
atp_1990_rankings_raw = pd.read_csv('tennis_atp/atp_rankings_90s.csv')
atp_2000_rankings_raw = pd.read_csv('tennis_atp/atp_rankings_00s.csv')
atp_2010_rankings_raw = pd.read_csv('tennis_atp/atp_rankings_10s.csv')
atp_2020_rankings_raw = pd.read_csv('tennis_atp/atp_rankings_20s.csv')
atp_current_rankings_raw = pd.read_csv('tennis_atp/atp_rankings_current.csv')
atp_players_raw = pd.read_csv('tennis_atp/atp_players.csv')


# Concatenate all raw data
ranking_dflist = [atp_1990_rankings_raw, atp_2000_rankings_raw,
                  atp_2010_rankings_raw, atp_2020_rankings_raw, atp_current_rankings_raw]
complete_ranking_raw = pd.concat(ranking_dflist)
complete_ranking_raw.rename(columns={'player': 'player_id'}, inplace=True)

# Top 1000 data
atp_top_1000 = atp_current_rankings_raw.loc[atp_current_rankings_raw['rank'] <= 1000]
atp_top_1000 = atp_top_1000[atp_top_1000['ranking_date'] == 20220627]
atp_top_1000.rename(columns={'player': 'player_id'}, inplace=True)

# Top 1000 players
atp_top_1000_players = pd.merge(
    atp_top_1000, atp_players_raw, on=['player_id'])
atp_top_1000_players = atp_top_1000_players.drop_duplicates(
    subset=['player_id']).drop(columns=['ranking_date'])
atp_top_1000_players.drop(columns=['wikidata_id'], inplace=True)

# Player timeseries
player_timeseries = pd.merge(
    complete_ranking_raw, atp_top_1000_players, on=['player_id'])
player_timeseries.drop(columns=[
                       'points_x', 'points_y', 'name_first', 'name_last', 'height', 'hand'], inplace=True)
player_timeseries['ranking_year'] = pd.to_datetime(
    player_timeseries.ranking_date, format='%Y%m%d').dt.year


# Top 1000 timeseries
top_1000_timeseries = player_timeseries.groupby(
    ['player_id', 'ranking_year']).agg({'min'})[['rank_x']].unstack(0)
...
for player_id in top_1000_timeseries.columns:
    player_ts = top_1000_timeseries[player_id].to_frame().rename(columns={
        player_id: 'rank'})
    player_ts['player_id'] = player_id
    player_historical = player_ts.reset_index().merge(atp_top_1000_players, on=[
        'player_id']).rename(columns={'rank_x': 'annual_best_rank', 'rank_y': 'current_rank'})
    player_historical = pd.merge(player_historical, player_timeseries[['ranking_date', 'player_id', 'ranking_year', 'rank_x']], left_on=[
        'player_id', 'ranking_year', 'annual_best_rank'], right_on=['player_id', 'ranking_year', 'rank_x'])
    player_historical.drop_duplicates(
        subset=['annual_best_rank', 'ranking_year'], keep='first', inplace=True)
    player_historical = player_historical[player_historical['annual_best_rank'].notna(
    )].reset_index(drop=True)
    player_historical['ranking_date'] = pd.to_datetime(
        player_historical['ranking_date'], format='%Y%m%d')
    # ((today.month, today.day) < (born.month, born.day))
    player_historical['dob'] = pd.to_datetime(
        player_historical['dob'], format='%Y%m%d')
    player_historical['age'] = player_historical.apply(lambda x: x['ranking_date'].year - x['dob'].year - (
        (x['ranking_date'].month, x['ranking_date'].day) < (x['dob'].month, x['dob'].day)), axis=1)
# today.year - born.year - ((today.month, today.day) < (born.month, born.day))
    top_1000_historical[player_historical.iat[0, 3]] = player_historical


# # All historical
# ind_players = atp_players_raw[atp_players_raw['ioc'] == 'IND']
...
# # Strength players
# strength_players = atp_players_raw[atp_players_raw['ioc'].isin(
#     ['USA', 'SUI', 'ESP', 'GBR'])]
...
#         player_historical['dob']).dt.year//10*10
...
#     'scrapy/Tennis_player_details_100.csv', on_bad_lines='skip')
...
# atp_players_raw['player_name'] = atp_players_raw['name_first'] + \
#     ' ' + atp_players_raw['name_last']
# atp_players_raw.dropna(subset=['dob'], how='all', inplace=True)
# atp_players_raw = atp_players_raw[~atp_players_raw.dob.str.startswith('t')]
# atp_players_raw['birth_year'] = atp_players_raw['dob'].apply(
#     lambda x: int(x)//10000)

# junior_players = atp_players_raw.merge(itf_juniors, on=[
